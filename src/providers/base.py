...
PricePrimaryTuple = tuple[
    Optional[float],
    Optional[int],
    Optional[int],
    Optional[float],
    Optional[float],
] # (price, sellers, listings, lowest_price, median_price)
...
class PriceProvider(Protocol):
    ...
    - fetch_primary_price: fonte "primaria" -> (price, sellers, listings, lowest_price, median_price)
    - fetch_secondary_breakdown: fonte "secondaria" -> (grades)
    """
    name: str

    def fetch_primary_price(self, item_id: Optional[int]) -> PricePrimaryTuple:
        """Ritorna (price, sellers, listings, lowest_price, median_price). Se non disponibili, usa None."""
        ...
