...
2. **Prices** ? `prep_prices_daily`
   ...
   - **Ottimizzazione fetch:** in `trainer.py` il download � limitato agli ultimi 400 giorni e proietta solo i campi necessari (riduce I/O da Mongo).
...
4. **Feature engineering** ? `add_features_daily`
   - Log price, ritorni 7/14/28/56d, volatilit� 28d, momentum 14d, liquidity (`price_to_listings`, `sellers_to_listings`), variazioni listings/sellers, spread, liquidity index, shock indicator.
...
- **Obiettivo**: raggruppare carte con profilo simile (rarit�, set, archetipo, illustrator, costo/potenza) per dare al modello un segnale di contesto stabile.
...
- **Perch�**: la dinamica di prezzo cambia molto tra carte low-cost e chase cards; tier separati evitano pattern incompatibili.
- **Regole default** (`MLConfig`): `low 0.5-10`, `mid 10-100`, `high 100-750`, `grail >= 750` (eur-equivalenti del prezzo corrente).
- **Uso**: il tier � calcolato sul prezzo attuale e seleziona il gruppo di modelli quantile.
...
2. Ricostruisce le feature al giorno pi� recente per ogni itemId.
...
- Aggiorna gli artifacts dopo modifiche a feature/clustering (`train_all`), altrimenti `predict_and_store` user� il vecchio schema.
